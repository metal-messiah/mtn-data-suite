      ...
      this.fileReader.onload = () => {
        if (this.fileReader.result) {
          if (this.file.name.includes('.xls')) {
            const wb: XLSX.WorkBook = XLSX.read(this.fileReader.result, {type: 'binary'});
            const sheetNames = Object.keys(wb.Sheets);

            if (sheetNames.length > 1) {
              const dialogRef = this.dialog.open(TabSelectDialogComponent, {data: {sheetNames}, disableClose: true});
              dialogRef.afterClosed().subscribe(selectedSheetName => {
                const worksheet = wb.Sheets[selectedSheetName];
                this.emit(this.file, XLSX.utils.sheet_to_csv(worksheet).replace(/\n/g, '\r\n'));
              });
            } else {
              this.emit(this.file, XLSX.utils.sheet_to_csv(wb.Sheets[sheetNames[0]]).replace(/\n/g, '\r\n'));
            }
          } else {
