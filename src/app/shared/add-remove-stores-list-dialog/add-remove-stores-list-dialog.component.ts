...
export class AddRemoveStoresListDialogComponent implements OnInit {
    ...
    this.storeListService
      ...
      .subscribe(
        page => {
          ...
          this.sortAlphabetically(this.storeLists, 'storeListName');
  ...
  handleFuzzySearch(output: [SimplifiedStoreList[], string]) {
    ...
    this.sortAlphabetically(this.storeListsDisplay, 'storeListName');
  }
  ...
  createNewList() {
    this.dialog
      .open(TextInputDialogComponent, { data: { title: 'New List', placeholder: 'New List Name' } })
      .afterClosed()
  ...
  submit() {
    if (this.type === AddRemoveType.ADD) {
      ...
      const obs = this.selectedListIds.map(listId =>
        this.storeListService.removeStoresFromStoreList(listId, this.storeIds)
      );
      forkJoin(obs).subscribe(() => {
