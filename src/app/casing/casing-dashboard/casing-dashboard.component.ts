...
import { ListManagerDialogComponent } from 'app/shared/list-manager-dialog/list-manager-dialog.component';
...
export class CasingDashboardComponent implements OnInit, OnDestroy {
    ...
    private getSitesInBounds(bounds) {
        ...
        return this.siteService
            .getSitesWithoutStoresInBounds(bounds)
            .pipe(tap((list) => this.siteMapLayer.setEntities(list)));
    }

    private getStoresInBounds(bounds) {
        const includeProjectIds = this.storeMapLayer.markerType === MarkerType.PROJECT_COMPLETION;
        return this.storeService
            .getStoresOfTypeInBounds(bounds, this.getFilteredStoreTypes(), includeProjectIds)
    ...
    editNewLocation(): void {
        ...
        this.ngZone.run(() => {
            ...
            this.geocoderService.reverseGeocode(coordinates).pipe(finalize(() => snackBarRef.dismiss())).subscribe(
                (newSite: Site) => {
    ...
    private createNewSite(site: Site) {
        this.ngZone.run(() => {
            ...
            this.siteService.create(site).pipe(finalize(() => this.cancelSiteCreation())).subscribe(
                () => {
   ...
   */
    ...
    saveMovedStore() {
        ...
        this.updateSiteCoordinates(movedStore.site.id, coordinates).subscribe(
            () => {
                this.selectedDashboardMode = CasingDashboardMode.DEFAULT;
            ...
            (err) =>
                this.errorService.handleServerError(
                    'Failed to update new Store location!',
                    err,
                    () => console.log('Cancel'),
                    () => this.saveMovedStore()
                )
    ...
    saveMovedSite() {
        ...
        this.updateSiteCoordinates(movedSite.id, coordinates).subscribe(
            () => {
                this.selectedDashboardMode = CasingDashboardMode.DEFAULT;
    ...
    updateSiteCoordinates(siteId: number, coordinates: Coordinates): Observable<Site> {
        return this.siteService.getOneById(siteId).pipe(
            mergeMap((site: Site) => {
                // Save updated values
    ...
    enableMultiSelect(): void {
        ...
        this.mapService.activateDrawingTools().subscribe((shape) => {
            ...
            if (geoJson.geometry.type === 'Point') {
                ...
                observable = this.storeService.getIdsInRadius(
                    latitude,
                    longitude,
                    radiusMeters,
                    this.casingDashboardService.filter
                );
            } else {
                observable = this.storeService.getIdsInShape(
            ...
            observable
                ...
                .subscribe((ids: { siteIds: number[]; storeIds: number[] }) => {
                    ...
                    if (
                        this.mapService.getZoom() <= this.DATA_POINT_ZOOM &&
                        this.mapService.getZoom() > this.MAX_DATA_ZOOM
                    ) {
   ...
   */
    openAssignmentDialog() {
        this.dialog.open(UserProfileSelectComponent).afterClosed().subscribe((user) => {
            if (user != null) {
    ...
    private assign(userId: number) {
        ...
        this.updating = true;
        this.storeService
            .assignToUser(selectedStoreIds, userId)
    ...
    private selectAllInShape(geoJsonString: string, clearPreviousSelection: boolean) {
        this.storeService
            .getIdsInShape(geoJsonString, this.casingDashboardService.filter)
            .subscribe((ids: { siteIds: number[]; storeIds: number[] }) => {
