...
export class LoadComponent implements OnInit {
  private readonly ST_SPREADSHEETS = 'spreadsheets';
  ...
  handleFile(file): void {
    ...
    if (type === 'application/json') {
      ...
      this.storageService
        .import(this.ST_SPREADSHEETS, storageItem, true, this.getNewProjectId())
        .subscribe(() => {
          // we imported it to local memory, now we need to update the list of stored projects with it
          this.storageService.getOne(this.ST_SPREADSHEETS).subscribe((item) => {
  ...
  removeStoredProject(id): void {
    ...
    if (answer) {
      ...
      this.storageService.set(this.ST_SPREADSHEETS, this.storedProjects).subscribe((r) => {
  ...
  shareStoredProject(id): void {
    ...
    this.storageService.export(this.ST_SPREADSHEETS, true, id);
  }
